    ...
    public void update() {
        ...
        if (followLuringGem && targetX != -1 && targetY != -1) {
            ...
            try {
                ...
                System.err.println("Error: Hero is not properly initialized or has been removed.");
            }
...
public class Fighter extends Character {
    ...
    public Image getSprite(SpriteLoader spriteLoader) {
        if (followLuringGem && targetX != -1 && targetY != -1) {
            return spriteLoader.getMonsterSprites()[targetX > xPosition ? 3 : 4];
        } else if (alternateAppearance) {
            return spriteLoader.getMonsterSprites()[sideOfHero ? 5 : 6]; // New sprites for alternate appearance
        } else {
            return spriteLoader.getMonsterSprites()[sideOfHero ? 3 : 4];
        }
    }
